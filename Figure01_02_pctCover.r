...
# set an output path
out_path <- "/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/FINAL_Jan2012/Figures/fig01_02/Fig01_02_Tables/"
...
for(i in 1:length(alf_names)){
	...
	if(alf_name == "CGCM31"){
		colnames(output) <- c("CCCMA Tundra PCH >60","CCCMA Tundra CAH >60") #
	}else{
		colnames(output) <- c("ECHAM5 Tundra PCH >60","ECHAM5 Tundra CAH >60")
	}
	print(alf_name)
	...
	for(j in 1:length(years)){
		...
		output[j,1] <- length(which(values(veg) == 1 & values(age) > 60 & values(pch.ras) == 1))/total.pch
		output[j,2] <- length(which(values(veg) == 1 & values(age) > 60 & values(cch.ras) == 1))/total.cch
	}
	rownames(output) <- years
	write.csv(output,file=paste(out_path,"Figure_01_02_",alf_name,"_pct.csv",sep=""))
}
...
# now lets ggplot it FOR TUNDRA GROUP
echam <- as.matrix(read.csv("/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/FINAL_Jan2012/Figures/fig01_02/Fig01_02_Tables/Figure_01_02_ECHAM_pct.csv", stringsAsFactors=FALSE))
cccma <- as.matrix(read.csv("/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/FINAL_Jan2012/Figures/fig01_02/Fig01_02_Tables/Figure_01_02_CGCM31_pct.csv",stringsAsFactors=FALSE))
...
# breaks=seq(round(min(pch[,2]),1),round(max(pch[,2],1)),by=0.20)
...
png(filename="/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/FINAL_Jan2012/Figures/fig01_02/Age_greater60_1950_2100_tundra_pch_012913_pct.png",width=800, height=800) 
pp + geom_line(size=1) + ggtitle("Tundra Cover with Age > 60 years \nPorcupine Herd (winter range)") + scale_y_continuous("Age > 60 years (% cover)")  + scale_x_continuous("time (years)", breaks=seq(1950,2100,20)) + scale_color_manual(values=c("black", "grey60"))+ opts(panel.background = theme_rect(fill="grey85", colour="grey85"))
dev.off()
...
# make a png graphics device
png(filename="/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/FINAL_Jan2012/Figures/fig01_02/Age_greater60_1950_2100_tundra_cah_012913_pct.png",width=800, height=800) 
pc + geom_line(size=1) + ggtitle("Tundra Cover with Age > 60 years \nCentral Arctic Herd (winter range)") + scale_y_continuous("Age > 60 years (% cover)") + scale_x_continuous("time (years)", breaks=seq(1950,2100,20)) + scale_color_manual(values=c("black", "grey60"))+ opts(panel.background = theme_rect(fill="grey85", colour="grey85"))
dev.off()
...
# png(filename="/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/Age_greater60_1950_2100_tundra_pch_2013.png")
...
# p + geom_line(size=1) + opts(title="Percent Cover Tundra >60 years - Porcupine Herd (winter range)") + scale_x_discrete(breaks=c(1950,2000,2050,2100)) + scale_y_discrete(breaks=c(min(as.integer(plotTable[,2])),median(as.integer(plotTable[,2])),max(as.integer(plotTable[,2])))) #breaks=quantile(as.numeric(plotTable[,1]))
# dev.off()
...
# png(filename="/workspace/UA/malindgren/projects/ALFRESCO/Caribou_Reanalysis/Output/Age_greater60_1950_2100_tundra_cah_2013.png")

# # p + geom_line(size=1) + scale_x_discrete(breaks=c(2000,2010,2020,2030,2040,2050,2060,2070,2080,2090), labels=c("2000","2010","2020","2030","2040","2050","2060","2070","2080","2090")) + scale_y_discrete(breaks=c(55,60,65,70,75), labels=c("55","60","65","70","75"))
# p + geom_line(size=1) + opts(title="Percent Cover Tundra >60 years - Central Arctic Herd (winter range)") + scale_x_discrete(breaks=c(1950,2000,2050,2100)) + scale_y_discrete(breaks=c(min(as.integer(plotTable[,2])),median(as.integer(plotTable[,2])),max(as.integer(plotTable[,2]))))
# dev.off()
